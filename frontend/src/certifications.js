...
export const loadCertificationsViewForUser = (userData) => {
    ...
    for (const certification of userData.certifications) {
        certList.innerHTML += `<div class="col-sm-2">
            <div class="card mb-3">
            <img src="http://localhost:3001/certification-bricks/${certification.id}.png" class="card-img-top to-lego" crossorigin="anonymous">
            <div class="card-body">
        ...
        for (const certRemove of certList.querySelectorAll('.certification-remove')) {
            certRemove.addEventListener('click', async function(e) {
                ...
                const req = await window.fetch(`http://localhost:3001/certifications/${user}/${id}`, {
                    method: 'DELETE'
                })
...
const addCertification = async (certification) => {
    ...
    const req = await window.fetch(`http://localhost:3001/certifications/${certification.user}`, {
        method: 'POST',
        body: data
    })
